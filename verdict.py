...
def generate_verdict(pass_results: list[dict[str, Any]], absence: list[dict[str, Any]]) -> dict[str, str]:
    ...
    if statuses and all(status == PASS_NOT_APPLICABLE for status in statuses):
        pass_ids = {result.get("pass_id") for result in pass_results}
    ...
    if statuses and all(status in {PASS_SATISFIED, PASS_SKIPPED} for status in statuses):
        return {
            "status": SUPPORTED,
